...
use crate::pixiv::{ArtworkListing, ArtworkPath, RawArtworkPath};
...
async fn artwork_response(
	...
	let listing =
		ArtworkListing::get_listing(path.language, path.id, &state.auth.access_token, &host, &state.client).await?;
	drop(state);

	let artwork = listing.to_template(path.image_index, host).unwrap();

	Ok((TypedHeader(CacheControl::new().with_no_cache()), Html(artwork)).into_response())
}
		...
		.ok()?
	{
		...
		if !bots.is_bot(user_agent.as_str()) {
			let redirect_uri = format!(
				...
				raw_path.language.as_ref().map(|l| format!("/{l}")).unwrap_or_default(),
				raw_path.id
			);
